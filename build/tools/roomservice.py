...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from CherishOS-Devices Github (http://github.com/CherishOS-Devices)." % device)

repositories = []

if not depsonly:
    githubreq = urllib.request.Request("https://raw.githubusercontent.com/CherishOS/official_devices/refs/heads/sixteen/devices.xml")
    try:
...
def get_default_revision():
    m = ElementTree.parse(".repo/manifests/snippets/cherish.xml")
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the cherish snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/cherish.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
